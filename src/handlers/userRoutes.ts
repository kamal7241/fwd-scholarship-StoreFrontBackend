...
import { OrderProductsStore, OrderProduct, OrderproductsDataBase } from './../services/dashboard/OrderProductsStore';
...
const createOrder = async (req : Request , res:Response , next:NextFunction)=>{
    try {
        ...
        await orderProductStore.addProduct({product_id:userOrder.products[0].id , order_id:createdOrder.id , quantity:userOrder.products[0].quantity});
        const products = await orderProductStore.getOrderProducts(createdOrder.id)
...
const getUserOrders = async (req : Request , res:Response , next:NextFunction)=>{
    try {
        ...
        const orders = await new OrderProductsStore().getUserOrders(userId) as OrderDetailsResponse[]
        orders.forEach( async (order:OrderDetailsResponse) =>{
            const products = await new OrderProductsStore().getOrderProducts(order.userid)
             order.products = products;
        })
        for (let i = 0; i < orders.length; i++) {
            const products = await new OrderProductsStore().getOrderProducts(orders[i].userid)
