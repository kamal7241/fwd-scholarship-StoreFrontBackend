...
export class UserStore  {
    async index():Promise<UserDataBase[]>{
        return  await asyncHandlerWrapper<UserDataBase[]>(async ():Promise<UserDataBase[]>=>{
    ...
    async create(user:User):Promise<UserDataBase>{
        return await asyncHandlerWrapper<UserDataBase>(async ()=>{
            ...
            const res = await connection.query(query , [user.firstname,user.lastname,user.password]);
            connection.release();
            return res.rows[0];
    ...
    async update(user:UserDataBase):Promise<UserDataBase>{
        return await asyncHandlerWrapper<UserDataBase>(async ()=>{
            ...
            const res = await connection.query(query , [user.firstname,user.lastname,user.password , user.id]);
            connection.release();
            return res.rows[0]; 
