...
export class ProductStore  {
    async index():Promise<Product[]>{
        return  await asyncHandlerWrapper<ProductDataBase[]>(async ():Promise<ProductDataBase[]>=>{
    ...
    async create(product:Product):Promise<ProductDataBase>{
        return await asyncHandlerWrapper<ProductDataBase>(async ()=>{
            ...
            const res = await connection.query(query , [product.name,product.price,product.category]);
            connection.release();
            return res.rows[0];
    ...
    async update(product:ProductDataBase):Promise<ProductDataBase>{
        return await asyncHandlerWrapper<ProductDataBase>(async ()=>{
            ...
            const res = await connection.query(query , [product.name,product.price,product.category , product.id]);
            connection.release();
            return res.rows[0]; 
