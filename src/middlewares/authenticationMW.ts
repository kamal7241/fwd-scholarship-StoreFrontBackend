...
export const authenticationMw = async(req:Request , res:Response , next:NextFunction)=>{
    ...
    if(result.rows.length){
        if(isPasswordMatchHashed(user.password , result.rows[0].password))
            next();
        next( new ResonseError(ErrorStatus.NotAuthorized , ErrorMessages.NotAuthenticated))
    }
    next( new ResonseError(ErrorStatus.NotAuthorized , ErrorMessages.NotAuthenticated))
}

export const authorizationMw = (req:Request , _res:Response , next:NextFunction)=>{
    const tokenHeader = req.headers.authorization as string;
    if(!tokenHeader)
        throw new ResonseError(ErrorStatus.NotAuthorized , ErrorMessages.NotAuthorized)
